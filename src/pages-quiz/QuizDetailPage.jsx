...
function QuizDetailPage() {
  ...
  const onCommentRemoveHandler = async (commentId) => {
    try {
      ...
      setComments((prevComments) =>
        prevComments.filter((comment) => comment.quizCommentId !== commentId)
      );
    } catch (error) {
  ...
  const onCommentUpdateHandler = async (commentId, updatedContent) => {
    try {
      await api.patch(
        `/quizzes/${quizId}/quizComments/${commentId}`,
        {
      ...
      setComments((prevComments) =>
        prevComments.map((comment) =>
          comment.quizCommentId === commentId
  ...
  return (
    <QuizDetailBoxStyle>
      ...
      {editableQuiz && (
        <div>
          ...
          <textarea
            ...
            onChange={(e) =>
              setEditableQuiz({ ...editableQuiz, content: e.target.value })
            }
            style={inputStyle}
          />
      ...
      <button onClick={handleMainButtonClick}>main으로 이동</button>
